...
async function assertIndependentApisAvailable(baseUrl: any, label: any): Promise<any> {
...
try {
  child = spawn(process.execPath, [path.resolve(".test-dist/server.js")], {
    cwd: path.resolve("."),
    env: {
      ...process.env,
      DATA_DIR: dataDir,
  ...
  const executedSchedule = await waitFor(async () => {
    ...
    return result.payload.find((item: any) => item.id === schedule.payload.id && item.lastResult) ?? null;
  });
