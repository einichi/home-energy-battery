...
for (const [edt, expected] of [[0x41, "generating"], [0x42, "stopped"], [0x43, "starting"], [0x44, "stopping"], [0x45, "idling"]]) {
  const decoded = decodeEnum({ host: "192.0.2.30", eoj: "0x027C01", epc: EPC.FUEL_CELL_GENERATION_STATUS, name: "status", raw: Buffer.from([Number(edt)]), mapping: EDT_TO_FUEL_CELL_STATUS });
...
for (const [edt, expected] of [[0x00, "grid_connected_reverse_flow_allowed"], [0x01, "independent"], [0x02, "grid_connected_reverse_flow_prohibited"]]) {
  const decoded = decodeEnum({ host: "192.0.2.30", eoj: "0x027C01", epc: EPC.FUEL_CELL_INTERCONNECTION_STATUS, name: "interconnection", raw: Buffer.from([Number(edt)]), mapping: EDT_TO_FUEL_CELL_INTERCONNECTION });
  assert.equal(decoded.human, expected);
}
