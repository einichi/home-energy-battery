...
async function waitFor(check: any, timeoutMs: any = 10_000): Promise<any> {
...
async function request(baseUrl: any, pathname: any, { method = "GET", body }: any = {}): Promise<any> {
...
function startTestServer(): any {
  const processHandle = spawn(process.execPath, [path.resolve(".test-dist/server.js")], {
    cwd: path.resolve("."),
    env: {
      ...process.env,
      DATA_DIR: dataDir,
  ...
  processHandle.stdout.on("data", (chunk: any) => { output += chunk; });
  processHandle.stderr.on("data", (chunk: any) => { output += chunk; });
...
try {
  ...
  assert.equal((await request(baseUrl, "/api/settings/charge", { method: "POST", body: {} })).response.status, 404);
  assert.equal((await request(baseUrl, "/api/actions/charge", { method: "POST", body: {} })).response.status, 404);
  assert.equal((await request(baseUrl, "/api/discovery", { method: "POST", body: {} })).response.status, 404);
  assert.equal((await request(baseUrl, "/api/gas-tariffs")).response.status, 404);
  assert.equal((await request(baseUrl, "/api/device-actions/fuel-cell-start", {
    method: "POST",
    body: {},
  })).response.status, 404);
  assert.equal((await fetch(`${baseUrl}/api/device-actions/charge`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: "[",
  })).status, 400);
  ...
  const charge = await request(baseUrl, "/api/device-actions/charge", {
    method: "POST",
    body: { targetWh: 500 },
  });
  ...
  const standby = await request(baseUrl, "/api/device-actions/set-mode", {
    method: "POST",
    body: { mode: "standby" },
  });
  ...
  const limit = await request(baseUrl, "/api/device-actions/discharge-limit", {
    method: "POST",
    body: { percent: 30 },
  });
  ...
  assert.ok(commandReceipts.payload.receipts.some((receipt: any) => receipt.commandId === limit.payload.commandId
    && receipt.action === "discharge-limit"
    && receipt.state === "succeeded"
    && receipt.events.some((event: any) => event.type === "acknowledged")
    && receipt.events.some((event: any) => event.type === "verifying")));
  ...
  assert.deepEqual(
    asynchronousReceipt.events.map((event: any) => event.type),
  ...
  for (const command of [
    ...
    assert.deepEqual(
      receipt.events.map((event: any) => event.type),
      ["succeeded", "verifying", "acknowledged", "sending", "requested"],
  ...
  const scheduleReceipt = (await request(baseUrl, "/api/command-receipts?limit=10")).payload.receipts
    .find((receipt: any) => receipt.source === "schedule" && receipt.action === "set-mode");
  assert.equal(scheduleReceipt.state, "succeeded");
  assert.ok(scheduleReceipt.events.some((event: any) => event.type === "acknowledged"));
  assert.ok(scheduleReceipt.events.some((event: any) => event.type === "verifying"));
  ...
  const backupReceipt = (await request(baseUrl, "/api/command-receipts?limit=10")).payload.receipts
    .find((receipt: any) => receipt.source === "backup-preparation" && receipt.action === "vendor-profile");
  assert.equal(backupReceipt.state, "succeeded");
  assert.ok(backupReceipt.events.some((event: any) => event.type === "acknowledged"));
  assert.ok(backupReceipt.events.some((event: any) => event.type === "verifying"));

  const blockedManualAction = await request(baseUrl, "/api/device-actions/vendor-profile", {
    method: "POST",
    body: { mode: "eco" },
  });
  ...
  const blockedReceipt = (await request(baseUrl, "/api/command-receipts?limit=10")).payload.receipts
    .find((receipt: any) => receipt.commandId === blockedManualAction.payload.commandId);
  ...
  const skippedSchedule = await waitFor(async () => {
    const schedules = await request(baseUrl, "/api/schedules");
    const item = schedules.payload.find((candidate: any) => candidate.id === blockedSchedule.payload.id);
  ...
  const persistedOverride = JSON.parse(String(persistedDatabase.prepare(
  ...
  assert.equal((await request(baseUrl, "/api/device-actions/charge", {
    method: "POST",
    body: { targetWh: 500 },
  })).response.status, 200);
  ...
  await waitFor(async () => {
    ...
    return rules.payload.find((item: any) => item.id === guard.payload.id)?.state?.awaitingRestore === true;
  });
  assert.equal((await request(baseUrl, "/api/status")).payload.energy.battery.operation_mode.value, "standby");
  ...
  const compatible = manualBackup.payload.backups.find((item: any) => item.kind === "manual");
  ...
  const incompatible = backupInventory.payload.backups.find((item: any) => item.filename === incompatibleFilename);
  ...
  assert.ok(restored.payload.backups.some((item: any) => item.kind === "pre-restore"));
  ...
  assert.equal(deleted.payload.backups.some((item: any) => item.filename === incompatibleFilename), false);
} finally {
