...
function tariffPeriod(rate: any, upper: any = 0): any {
  return {
    hyo: [{
      ryokinhyo: { val: "A表" },
...
const apiDocument: Record<string, any> = {
  kateiyo: {
    enefarm: {
...
await assert.rejects(
  importTokyoGasTariff({ billingMonth: "July", fetchImpl: (async () => ({ ok: true, json: async () => apiDocument })) as any }),
  /YYYY-MM/,
);
await assert.rejects(
  importTokyoGasTariff({ billingMonth: "2026-07", region: "invalid", fetchImpl: (async () => ({ ok: true, json: async () => apiDocument })) as any }),
