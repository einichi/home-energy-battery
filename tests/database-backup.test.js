...
try {
  ...
  assert.ok(progress.some((value) => value.phase === "validating"));
  ...
  try {
    assert.equal(
      JSON.parse(restored.prepare("SELECT value FROM metadata WHERE key = 'architectureVersion'").get().value),
      ARCHITECTURE_VERSION,
    );
    assert.deepEqual(
      JSON.parse(restored.prepare("SELECT payload_json FROM application_documents WHERE key = 'config'").get().payload_json),
