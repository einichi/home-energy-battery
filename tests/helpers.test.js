...
test("clearStaleScheduleRuns clears only schedules without an active run", () => {
...
test("cleanConfig migrates legacy off-peak settings into rate bands", () => {
...
test("cleanConfig applies simple-mode defaults", () => {
...
test("cleanConfig clamps numeric fields", () => {
...
test("normalizeDashboardWidgets merges overrides and drops unknown widgets", () => {
...
test("parseJsonWithContext reports location and recoverConcatenatedJsonValue recovers", () => {
  assert.throws(
...
test("normalizeSubnets validates and dedupes /24 CIDRs", () => {
  assert.deepEqual(normalizeSubnets(["192.168.1.0/24", "bad", "192.168.1.0/24"]), ["192.168.1.0/24"]);
});

test("normalizeRateBands + rateForTimestamp resolve the active band", () => {
...
test("sampleFromStatus computes solar/grid energy and savings", () => {
  const bands = normalizeRateBands({
    rateBands: [
      { start: "23:00", end: "07:00", yenPerKwh: 20, label: "Night" },
      { start: "07:00", end: "23:00", yenPerKwh: 40, label: "Day" },
...
test("summarizeSamples aggregates direct and legacy power fields", () => {
...
test("cleanAutomationRule applies guard defaults and splits config from state", () => {
...
test("evaluateAutomationRule skips when conditions are not met or demand is unavailable", async () => {
  const rule = cleanAutomationRule({ enabled: true, conditions: { breakerAmps: 40, reserveAmps: 5, source: "houseDemandW" } });
...
test("evaluateAutomationRule accounts for battery charging without double counting grid import", async () => {
...
test("evaluateAutomationRule guards restore against tripping the breaker", async () => {
...
test("evaluateAutomationRule logs repeated high-demand restore checks", async () => {
