...
function hexByte(value: any): any {
  return Number(value).toString(16).padStart(2, "0").toUpperCase();
}

function rawUnsigned(value: any, bytes: any = 4): any {
  if (!Number.isFinite(Number(value))) return null;
  return `0x${Math.max(0, Math.round(Number(value))).toString(16).padStart(bytes * 2, "0").toUpperCase()}`;
}

function metric({ host, eoj, epc, name, value, unit = null, raw = null }: any): any {
  return {
    host,
    eoj,
    epc,
...
function enumMetric({ host, eoj, epc, name, value, raw = "0x00" }: any): any {
  return metric({ host, eoj, epc, name, value, raw });
}

function normalizedHostList(value: any): any {
  if (value === undefined || value === null || value === "") return [];
  return (Array.isArray(value) ? value : [value]).map(String);
}

function profileRaw(profile: any): any {
  return `0x${hexByte(({ osaifu: 0x02, eco: 0x03, backup: 0x20 } as Record<string, number>)[profile] ?? 0xff)}`;
}

function modeRaw(mode: any): any {
  return `0x${hexByte(({ rapid_charging: 0x41, charging: 0x42, charge: 0x42, discharging: 0x43, discharge: 0x43, standby: 0x44, auto: 0x46 } as Record<string, number>)[mode] ?? 0x40)}`;
}

function windowRaw(window: any): any {
  return `0x${hexByte(window.startHour)}00${hexByte(window.endHour)}00`;
}

function decodeWindow(window: any): any {
...
function simulatedFuelCellMetric(host: any, role: any, fuelCell: any, available: any): any {
  const value = (item: any) => available ? item : null;
...
export function createDeviceSimulator(options: any = {}): any {
  const preset = typeof options.scenario === "string" ? (DEVICE_SCENARIOS as Record<string, any>)[options.scenario] : options.scenario;
  ...
  function hostAvailable(host: any): any {
    return host && !state.unavailableHosts.includes(host);
  }

  function failNext(command: any, { host = null, message = `${command} simulated failure`, times = 1 }: any = {}): any {
    faults.push({ command, host, message, kind: "throw", remaining: Math.max(1, Number(times) || 1) });
  }

  function rejectNext(command: any, { host = null, esv = "SetC_SNA", times = 1 }: any = {}): any {
    faults.push({ command, host, esv, kind: "reject", remaining: Math.max(1, Number(times) || 1) });
  }

  function setState(patch: any): any {
    state = merge(state, patch);
    return snapshot();
  }

  function setHostAvailable(host: any, available: any): any {
  ...
  function advance(milliseconds: any): any {
  ...
  function batterySetting(command: any, positional: any): any {
  ...
  function energyStatus(args: any): any {
    ...
    const batteryValue = (value: any) => batteryAvailable ? value : null;
    ...
    const proxyHosts = normalizedHostList(args["fuel-cell-proxy-host"] ?? state.fuelCell.proxyHosts)
      .filter((host: any) => host !== primaryHost);
    const errors: any[] = [];
    const addUnavailableErrors = (host: any, eoj: any, epcs: any) => {
    ...
    return {
      ...
      fuel_cells: args["no-fuel-cell"] === true ? [] : [
        ...
        ...proxyHosts.map((host: any) => simulatedFuelCellMetric(host, "proxy", state.fuelCell, hostAvailable(host))),
      ],
    };
  }

  function meterStatus(args: any): any {
    ...
    return {
      ...
      channel_power: { host, eoj: METER_EOJ, epc: "0xB7", name: "channel_instant_power", raw: "0x00", decoded: { start: circuits[0]?.channel ?? null, range: circuits.length, channels: circuits.map((circuit: any) => ({ channel: circuit.channel, value: circuit.instantPowerW, unit: "W", human: `${circuit.instantPowerW} W` })) } },
      channel_energy: { host, eoj: METER_EOJ, epc: "0xB3", name: "channel_cumulative_energy", raw: "0x00", decoded: { start: circuits[0]?.channel ?? null, range: circuits.length, channels: circuits.map((circuit: any) => ({ channel: circuit.channel, count: Math.round(circuit.cumulativeKwh / state.meter.cumulativeUnitKwh), value: circuit.cumulativeKwh, unit: "kWh", human: `${circuit.cumulativeKwh.toFixed(2)} kWh` })) } },
    };
  }
  ...
  async function execute(command: any, args: any = {}, positional: any = []): Promise<any> {
    const call: Record<string, any> = { command, args: clone(args), positional: clone(positional), at: new Date().toISOString() };
    calls.push(call);
    const fault = faults.find((item: any) => item.remaining > 0 && item.command === command && (!item.host || item.host === args.host));
    if (fault) {
      fault.remaining -= 1;
    ...
    if (Number(behavior.delayMs) > 0) {
      await new Promise((resolve: any) => setTimeout(resolve, Number(behavior.delayMs)));
    }
...
export async function createDeviceCommandAdapter({ environment = process.env }: any = {}): Promise<any> {
