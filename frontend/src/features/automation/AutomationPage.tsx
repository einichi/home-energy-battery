...
function Timeline({ items = [] }: { items?: AdaptiveTimelineItem[] }) {
  if (!items.length) return <div className="automation-empty" role="status">No plan timeline is available yet. Complete setup or recalculate after forecast data becomes available.</div>;
  const start = new Date(items[0].start).getTime();
  const end = new Date(items.at(-1)?.end ?? items[0].end).getTime();
  const duration = Math.max(1, end - start);
  return (
    <div className="automation-timeline" aria-label="Today and tonight automation plan">
      ...
      <div className="automation-timeline-legend"><span data-tone="charge">Planned charging</span><span data-tone="discount">Discounted period</span><span data-tone="away">Away assumptions</span><span data-tone="observe">Forecast only</span></div>
    </div>
  );
}
...
function Activity({ adaptive, guard, receipts }: { adaptive: AdaptiveChargingStatus | null; guard: AutomationRule | null; receipts: CommandReceipt[] }) {
  ...
  return (
    <section className="panel automation-activity" aria-labelledby="automation-activity-heading">
      <div className="section-heading"><div><p className="eyebrow">Audit trail</p><h2 id="automation-activity-heading">Recent activity</h2></div></div>
      {rows.length ? <div className="table-scroll"><table><thead><tr><th>Severity</th><th>Time</th><th>Actor</th><th>Action and reason</th><th>Result</th></tr></thead><tbody>{rows.map((entry, index) => <tr key={`${entry.at}:${entry.actor}:${index}`}><td><span className="event-severity" data-kind={entry.kind}>{entry.kind}</span></td><td>{formatDateTime(entry.at)}</td><td>{entry.actor}</td><td>{entry.message}</td><td>{entry.result}</td></tr>)}</tbody></table></div> : <p className="automation-empty">No automation activity has been recorded yet.</p>}
    </section>
  );
}
  ...
  return (
    <div className="modal-backdrop" role="presentation">
      <section className="command-dialog panel" role="dialog" aria-modal="true" aria-labelledby="automation-confirm-title">
        <p className="eyebrow">Review automation change</p>
        <h2 id="automation-confirm-title">{confirmation.title}</h2>
        <p className="impact-note">{confirmation.impact}</p>
        <div className="button-row dialog-actions">
          <button className="button secondary" type="button" disabled={busy} onClick={close}>Cancel</button>
  ...
  return (
    <section className="panel away-workspace" aria-labelledby="away-summary-heading">
      <div className="section-heading">
        <div><p className="eyebrow">Occupancy context</p><h2 id="away-summary-heading">Away schedule</h2></div>
        <span className="automation-status-pill" data-tone={away.state === "away" ? "warning" : "positive"}>{away.state === "away" ? "Away" : "Home"}</span>
      </div>
      <p className="section-copy">Away periods reduce forecast demand assumptions and queue a fresh charging plan. They do not directly control household equipment.</p>
      {away.active ? <div className="away-active"><div><strong>Away now</strong><span>Expected home {formatDateTime(away.active.until)}</span></div><div className="button-row"><button className="quiet-button" type="button" disabled={busy !== null} onClick={() => edit(away.active as AwayPeriod, "extend")}>Extend</button><button className="button primary" type="button" disabled={busy !== null} onClick={() => void onBackHome(away.active as AwayPeriod)}>Back home</button></div></div> : null}
      <form className="away-form" onSubmit={(event) => void submit(event)}>
        <div className="section-heading away-form-heading"><div><h3>{draft.mode === "edit" ? "Edit Away period" : draft.mode === "extend" ? "Extend Away period" : draft.source === "manual" ? "Away now" : "Schedule Away period"}</h3><p>{draft.source === "manual" ? "Confirm when you expect to return before starting Away mode." : "The plan will use lower demand assumptions for this period."}</p></div>{draft.mode !== "create" || draft.source === "manual" ? <button className="text-button" type="button" onClick={reset}>Cancel</button> : null}</div>
        <div className="away-form-grid">
          <label className="field">From<input aria-label="Away from" type="datetime-local" required disabled={draft.mode === "extend"} value={draft.from} onChange={(event) => setDraft((current) => ({ ...current, from: event.target.value }))} /></label>
          <label className="field">Until<input aria-label="Away until" type="datetime-local" required value={draft.until} onChange={(event) => setDraft((current) => ({ ...current, until: event.target.value }))} /></label>
          <div className="away-form-actions"><button className="quiet-button" type="button" disabled={busy !== null || away.active !== null} onClick={beginNow}>Away now</button><button className="button primary" type="submit" disabled={busy !== null}>{busy === "away" ? "Saving…" : draft.mode === "edit" ? "Save period" : draft.mode === "extend" ? "Save extension" : draft.source === "manual" ? "Start Away period" : "Add period"}</button></div>
        </div>
      </form>
      ...
      <div className="away-period-list">
        ...
        {away.periods.filter((period) => period.status !== "active").length ? away.periods.filter((period) => period.status !== "active").map((period) => <article key={period.id}><div><strong>{formatDateTime(period.from)}</strong><span>Until {formatDateTime(period.until)} · {period.source === "manual" ? "Started manually" : "Scheduled"}</span></div><div className="button-row">{deleteCandidate?.id === period.id ? <><span className="delete-prompt">Remove this period?</span><button className="quiet-button" type="button" onClick={() => setDeleteCandidate(null)}>Keep</button><button className="button danger" type="button" disabled={busy !== null} onClick={() => void onDelete(period).then(() => setDeleteCandidate(null))}>Remove</button></> : <><button className="quiet-button" type="button" disabled={busy !== null} onClick={() => edit(period, "edit")}>Edit</button><button className="quiet-button danger" type="button" disabled={busy !== null} onClick={() => setDeleteCandidate(period)}>Delete</button></>}</div></article>) : <p className="automation-empty compact">No future Away periods are scheduled.</p>}
      </div>
      <p className="panel-note">Normal demand assumptions resume after the return time plus a {away.returnBufferMinutes ?? 30}-minute buffer.</p>
    </section>
  );
}
  ...
  return (
    <section className="automation-view-stack" aria-label="Automation performance">
      <section className="panel">
        <div className="section-heading"><div><p className="eyebrow">Current planning evidence</p><h2>Forecast and control performance</h2><p className="section-copy">Planning values are estimates. Outcome rows distinguish forecasts from recorded or partially estimated results.</p></div></div>
        <div className="performance-domain-grid">
          <article><span>Solar forecast</span><strong>{formatEnergy(plan?.predictedSolarKwh)}</strong><small>Estimated for the current horizon · {solarOutcomes.length} completed days</small></article>
          <article><span>Demand forecast</span><strong>{formatEnergy(plan?.predictedDemandKwh)}</strong><small>Estimated from {demandDays} valid historical days</small></article>
          <article><span>Battery plan</span><strong>{formatEnergy(plan?.plannedChargeKwh)}</strong><small>Estimated grid charge · {windowOutcomes.length} completed windows</small></article>
          <article><span>Ene-Farm forecast</span><strong>{formatEnergy(plan?.predictedFuelCellKwh)}</strong><small>Estimated contribution · {fuelOutcomes.length} completed intervals</small></article>
        </div>
      </section>
      <section className="performance-detail-grid">
        <article className="panel performance-evidence"><div className="section-heading"><div><p className="eyebrow">Solar</p><h2>Forecast outcomes</h2></div><span className="quality-label">{adaptive?.solarForecastAccuracy?.learned ? "Calibrated estimate" : "Learning estimate"}</span></div><dl className="performance-summary"><div><dt>Evidence</dt><dd>{adaptive?.solarForecastAccuracy?.sampleCount ?? 0} days</dd></div><div><dt>Mean absolute error</dt><dd>{solarMae.length ? formatEnergy(solarMae.reduce((sum, value) => sum + value, 0) / solarMae.length) : "—"}</dd></div></dl>{solarOutcomes.length ? <div className="table-scroll"><table><thead><tr><th>Date</th><th>Issued estimate</th><th>Planning estimate</th><th>Recorded generation</th><th>Error</th></tr></thead><tbody>{[...solarOutcomes].reverse().slice(0, 8).map((outcome, index) => <tr key={`${outcome.targetDate}:${index}`}><td>{outcome.targetDate ?? "—"}</td><td>{formatEnergy(outcome.predictedKwh)}</td><td>{formatEnergy(outcome.planningKwh)}</td><td>{formatEnergy(outcome.actualKwh)}</td><td>{signedEnergy(outcome.errorKwh)}</td></tr>)}</tbody></table></div> : <p className="automation-empty compact">No completed solar forecast days yet.</p>}</article>
        <article className="panel performance-evidence"><div className="section-heading"><div><p className="eyebrow">Demand</p><h2>Historical model</h2></div><span className="quality-label">Estimated</span></div><dl className="performance-summary"><div><dt>Recorded days</dt><dd>{plan?.demandHistory?.recordedDayCount ?? 0}</dd></div><div><dt>Valid days</dt><dd>{plan?.demandHistory?.validDayCount ?? 0}</dd></div><div><dt>Recent comparisons</dt><dd>{plan?.demandHistory?.recentComparableDayCount ?? 0}</dd></div><div><dt>Seasonal comparisons</dt><dd>{plan?.demandHistory?.seasonalComparableDayCount ?? 0}</dd></div></dl><p className="panel-note">The current API exposes demand-model evidence and the active estimate, but not a separate settled demand-error series.</p></article>
        <article className="panel performance-evidence"><div className="section-heading"><div><p className="eyebrow">Battery</p><h2>Charging-window outcomes</h2></div><span className="quality-label">Recorded + estimated</span></div>{windowOutcomes.length ? <div className="table-scroll"><table><thead><tr><th>Window</th><th>Planned</th><th>Delivered</th><th>SOC</th><th>Result</th></tr></thead><tbody>{windowOutcomes.slice(0, 8).map((outcome, index) => <tr key={`${outcome.key}:${index}`}><td>{formatDateTime(outcome.windowStart)}<small>{outcome.label ?? "Discounted"}</small></td><td>{formatWh(outcome.plannedWh)}</td><td>{formatWh(outcome.deliveredWh)}{Number(outcome.estimatedDeliveryWh) > 0 ? <small>{formatWh(outcome.estimatedDeliveryWh)} boundary estimate</small> : null}</td><td>{formatPercent(outcome.startSocPercent)} → {formatPercent(outcome.endSocPercent)}</td><td>{outcome.socTargetReached ? "Target reached" : Number(outcome.unmetWh) > 0 ? `${formatWh(outcome.unmetWh)} short` : "Completed"}</td></tr>)}</tbody></table></div> : <p className="automation-empty compact">No completed charging windows yet.</p>}</article>
        <article className="panel performance-evidence"><div className="section-heading"><div><p className="eyebrow">Ene-Farm</p><h2>Forecast outcomes</h2></div><span className="quality-label">Estimated vs recorded</span></div><dl className="performance-summary"><div><dt>Completed intervals</dt><dd>{fuelOutcomes.length}</dd></div><div><dt>Mean absolute error</dt><dd>{fuelMae.length ? formatEnergy(fuelMae.reduce((sum, value) => sum + value, 0) / fuelMae.length) : "—"}</dd></div></dl>{fuelOutcomes.length ? <div className="table-scroll"><table><thead><tr><th>Interval</th><th>Median estimate</th><th>Recorded output</th><th>Error</th><th>Plan influence</th></tr></thead><tbody>{fuelOutcomes.slice(0, 8).map((outcome, index) => <tr key={`${outcome.targetStart}:${index}`}><td>{formatTime(outcome.start.toISOString())}–{formatTime(outcome.end.toISOString())}</td><td>{formatEnergy(outcome.predictedKwh)}</td><td>{formatEnergy(outcome.actualKwh)}</td><td>{signedEnergy(outcome.errorKwh)}</td><td>{outcome.influence ?? "—"}</td></tr>)}</tbody></table></div> : <p className="automation-empty compact">No completed Ene-Farm forecast intervals yet.</p>}</article>
      </section>
      <section className="panel model-progress"><div className="section-heading"><div><p className="eyebrow">Battery learning</p><h2>Model evidence</h2></div><span className="quality-label">{adaptive?.batteryModel?.status ?? "Unavailable"} · v{adaptive?.batteryModel?.version ?? "—"}</span></div><dl><div><dt>Charge observations</dt><dd>{adaptive?.batteryModel?.charge?.acceptedObservationCount ?? 0}</dd></div><div><dt>Discharge observations</dt><dd>{adaptive?.batteryModel?.discharge?.acceptedObservationCount ?? 0}</dd></div><div><dt>Charge-power samples</dt><dd>{adaptive?.batteryModel?.power?.sampleCount ?? 0}</dd></div><div><dt>Charging sessions</dt><dd>{adaptive?.batteryModel?.power?.sessionCount ?? 0}</dd></div></dl></section>
    </section>
  );
}
...
export function AutomationPage() {
  ...
  return (
    <main className="page automation-page">
      <header className="page-heading"><div><p className="eyebrow">Decide</p><h1>Automation</h1><p>Understand what the system will do next, why it chose that action, and which protections can intervene.</p></div><button className="quiet-button" type="button" onClick={refresh} disabled={loading || manualRefreshing}>{loading ? "Loading…" : manualRefreshing ? "Refreshing…" : "Refresh"}</button></header>
      {error ? <div className="status-banner" data-severity="critical">Automation: {error}</div> : null}

      <nav className="automation-tabs" aria-label="Automation views">{(["plan", "performance", "configuration"] as AutomationView[]).map((item) => <button key={item} type="button" aria-pressed={view === item} onClick={() => setView(item)}>{item[0].toUpperCase() + item.slice(1)}</button>)}</nav>

      {view === "plan" ? <>
        <section className="automation-summary-grid" aria-label="Automation summary">
          <article className="panel automation-master"><p className="eyebrow">System state</p><div className="automation-master-state"><h2>{state.label}</h2><span data-tone={state.tone} /></div><p>{state.detail}</p></article>
          <article className="panel automation-next"><p className="eyebrow">Next action</p><h2>{next.title}</h2><p>{next.reason}</p>{adaptive?.plan?.targetSunset ? <small>Plan horizon: {formatDateTime(adaptive.plan.targetSunset)}</small> : null}</article>
        </section>

        <section className="panel automation-protections" aria-labelledby="protections-heading"><div className="section-heading"><div><p className="eyebrow">Safety</p><h2 id="protections-heading">Active protections</h2></div></div><div className="protection-grid"><article><span>Demand Guard</span><strong>{guard?.enabled ? guard.state?.awaitingRestore ? "Intervening" : "Monitoring" : "Off"}</strong><small>{guard?.enabled ? `${formatPower(headroom)} headroom before intervention` : "Breaker protection is disabled"}</small></article><article><span>Breaker contract</span><strong>{formatPower(Number.isFinite(breakerWatts) ? breakerWatts : null)}</strong><small>{guard?.conditions?.breakerAmps ?? "—"} A at {guard?.conditions?.breakerVoltage ?? "—"} V</small></article><article><span>Current grid import</span><strong>{formatPower(gridImport)}</strong><small>{Number.isFinite(thresholdWatts) ? `Guard threshold ${formatPower(thresholdWatts)}` : "Guard threshold unavailable"}</small></article><article><span>Disaster Prep priority</span><strong>{status?.batteryStrategy?.kind === "backup-preparation" ? "Active" : "Inactive"}</strong><small>{status?.batteryStrategy?.kind === "backup-preparation" ? "Backup readiness owns the battery; Adaptive Charging can observe but cannot issue commands." : "Adaptive Charging may operate when its plan and safety checks allow."}</small></article><article className="protection-owner"><span>Operational owner</span><strong>{status?.batteryStrategy?.title ?? "Unavailable"}</strong><small>{status?.batteryStrategy?.description ?? "Waiting for battery strategy"}</small></article></div></section>

        <section className="panel automation-plan" aria-labelledby="automation-plan-heading"><div className="section-heading"><div><p className="eyebrow">Today and tonight</p><h2 id="automation-plan-heading">Shared automation timeline</h2></div><div className="automation-plan-actions"><span className="sample-count">{adaptive?.plan?.timeline?.length ?? 0} intervals</span><button className="quiet-button" type="button" disabled={busy !== null || !adaptive?.enabled || !configurationReady} onClick={() => void runPlanAction("recalculate")}>{busy === "recalculate" ? "Recalculating…" : "Recalculate plan"}</button>{adaptive?.paused ? <button className="button primary" type="button" disabled={busy !== null || !configurationReady} onClick={() => setConfirmation({ title: "Resume Adaptive Charging", impact: "Adaptive Charging will clear the current pause and recalculate its plan. It may resume control of battery charging when the next eligible discounted window begins. Active Disaster Prep still takes priority.", confirmLabel: "Resume automation", run: () => runPlanAction("resume") })}>Resume</button> : null}</div></div><Timeline items={adaptive?.plan?.timeline} /><div className="selected-windows"><h3>Selected discounted windows</h3>{adaptive?.plan?.slots?.length ? <div>{adaptive.plan.slots.map((slot, index) => <article key={`${slot.start}:${index}`}><span>{slot.label ?? "Discounted rate"}</span><strong>{formatTime(slot.start)}–{formatTime(slot.end)}</strong><small>{formatWh(slot.targetWh)} planned{slot.targetSocPercent == null ? "" : ` · target ${formatPercent(slot.targetSocPercent)}`}</small></article>)}</div> : <p>No discounted charging window is selected for this plan.</p>}</div>{planResult ? <p className={`inline-save-result ${planResult.ok ? "success" : "failure"}`} role={planResult.ok ? "status" : "alert"}>{planResult.message}</p> : null}</section>

        <div className="automation-context-grid"><AwayWorkspace away={away} busy={busy} result={awayResult} onSubmit={mutateAway} onDelete={removeAway} onBackHome={backHome} /><section className="panel automation-assumptions" aria-labelledby="assumptions-heading"><div className="section-heading"><div><p className="eyebrow">Planning basis</p><h2 id="assumptions-heading">Forecast assumptions</h2></div></div><dl><div><dt>Current SOC</dt><dd>{formatPercent(adaptive?.plan?.currentSocPercent)}</dd></div><div><dt>Target SOC</dt><dd>{formatPercent(adaptive?.plan?.targetSocPercent)}</dd></div><div><dt>Expected sunset SOC</dt><dd>{formatPercent(adaptive?.plan?.expectedSunsetSocPercent)}</dd></div><div><dt>Forecast confidence</dt><dd>{adaptive?.solarForecastAccuracy?.learned ? `Calibrated · ${adaptive.solarForecastAccuracy.sampleCount ?? 0} days` : `Initial model · ${adaptive?.solarForecastAccuracy?.sampleCount ?? 0} days`}</dd></div><div><dt>Forecast solar</dt><dd>{formatEnergy(adaptive?.plan?.predictedSolarKwh)}</dd></div><div><dt>Forecast demand</dt><dd>{formatEnergy(adaptive?.plan?.predictedDemandKwh)}</dd></div><div><dt>Forecast Ene-Farm</dt><dd>{formatEnergy(adaptive?.plan?.predictedFuelCellKwh)}</dd></div><div><dt>Forecast surplus</dt><dd>{formatEnergy(adaptive?.plan?.predictedSurplusKwh)}</dd></div></dl></section></div>
        <Activity adaptive={adaptive} guard={guard} receipts={receipts} />
      </> : null}

      {view === "performance" ? <PerformanceView adaptive={adaptive} /> : null}

      {view === "configuration" ? <section className="automation-view-stack" aria-label="Automation configuration"><section className="panel setup-checklist"><div className="section-heading"><div><p className="eyebrow">Prerequisites</p><h2>Setup checklist</h2></div><span className="sample-count">{checks.filter((item) => item.ready).length}/{checks.length} ready</span></div><ul>{checks.map((item) => <li key={item.label} data-ready={item.ready}><i aria-hidden="true">{item.ready ? "✓" : "!"}</i><div><strong>{item.label}</strong><span>{item.detail}</span><a href={item.href}>{item.action}<span aria-hidden="true"> →</span></a></div></li>)}</ul></section>
        <form id="adaptive-settings" className="panel automation-settings-form" key={`adaptive:${JSON.stringify(config?.adaptiveCharging)}:${JSON.stringify(config?.batteryCapabilities)}`} onSubmit={submitAdaptive}><div className="section-heading"><div><p className="eyebrow">Planning settings</p><h2>Adaptive Charging configuration</h2><p className="section-copy">Changes invalidate the current plan and queue a recalculation.</p></div></div><label className="automation-toggle"><input name="enabled" type="checkbox" defaultChecked={config?.adaptiveCharging?.enabled === true} /><span><strong>Enable Adaptive Charging</strong><small>Allow the application to select and operate discounted charging windows.</small></span></label><div className="automation-form-grid"><label className="field">Latitude<input name="latitude" type="number" min="-90" max="90" step="0.000001" required defaultValue={config?.adaptiveCharging?.latitude ?? ""} /></label><label className="field">Longitude<input name="longitude" type="number" min="-180" max="180" step="0.000001" required defaultValue={config?.adaptiveCharging?.longitude ?? ""} /></label><label className="field">Array peak capacity<div className="input-suffix"><input name="arrayPeakKw" type="number" min="0.1" step="0.1" required defaultValue={config?.adaptiveCharging?.arrayPeakKw ?? ""} /><span>kW</span></div></label><label className="field">Panel tilt<div className="input-suffix"><input name="panelTiltDegrees" type="number" min="0" max="90" step="1" required defaultValue={config?.adaptiveCharging?.panelTiltDegrees ?? 30} /><span>°</span></div></label><label className="field">Panel azimuth<div className="input-suffix"><input name="panelAzimuthDegrees" type="number" min="-180" max="180" step="1" required defaultValue={config?.adaptiveCharging?.panelAzimuthDegrees ?? 0} /><span>°</span></div></label><label className="field">Initial system loss<div className="input-suffix"><input name="systemLossPercent" type="number" min="0" max="50" step="1" required defaultValue={config?.adaptiveCharging?.systemLossPercent ?? 14} /><span>%</span></div></label><label className="field">Maximum off-peak SOC<div className="input-suffix"><input name="targetSocPercent" type="number" min="50" max="100" step="1" required defaultValue={config?.adaptiveCharging?.targetSocPercent ?? 100} /><span>%</span></div></label><label className="field">Forecast confidence margin<div className="input-suffix"><input name="forecastMarginPercent" type="number" min="0" max="50" step="1" required defaultValue={config?.adaptiveCharging?.forecastMarginPercent ?? 10} /><span>%</span></div></label><label className="field">Usable battery capacity<div className="input-suffix"><input name="usableCapacityKwh" type="number" min="0.1" step="0.1" required defaultValue={config?.batteryCapabilities?.usableCapacityKwh ?? ""} /><span>kWh</span></div></label><label className="field">Maximum charge power<div className="input-suffix"><input name="maximumChargeWatts" type="number" min="50" step="1" required defaultValue={config?.batteryCapabilities?.maximumChargeWatts ?? ""} /><span>W</span></div></label></div><div className="form-footer"><button className="button primary" type="submit" disabled={busy !== null || !config}>{busy === "adaptive-config" ? "Saving…" : "Review and save"}</button>{adaptiveResult ? <p className={`inline-save-result ${adaptiveResult.ok ? "success" : "failure"}`} role={adaptiveResult.ok ? "status" : "alert"}>{adaptiveResult.message}</p> : null}</div></form>
        <form className="panel automation-settings-form" key={`guard:${JSON.stringify(guard)}`} onSubmit={submitGuard}><div className="section-heading"><div><p className="eyebrow">Breaker protection</p><h2>Demand Guard configuration</h2><p className="section-copy">Demand Guard pauses battery charging before grid import reaches the configured breaker margin.</p></div></div><label className="automation-toggle"><input name="enabled" type="checkbox" defaultChecked={guard?.enabled === true} /><span><strong>Enable Demand Guard</strong><small>Permit Standby and Auto mode changes when protecting breaker headroom.</small></span></label><label className="automation-toggle compact"><input name="dashboardWarningEnabled" type="checkbox" defaultChecked={guard?.dashboardWarningEnabled !== false} /><span><strong>Show breaker-risk warning</strong><small>Surface approaching interventions in global status.</small></span></label><div className="automation-form-grid guard"><label className="field">Breaker limit<div className="input-suffix"><input name="breakerAmps" type="number" min="1" max="400" step="1" required defaultValue={guard?.conditions?.breakerAmps ?? 40} /><span>A</span></div></label><label className="field">Reserved headroom<div className="input-suffix"><input name="reserveAmps" type="number" min="0" max="200" step="1" required defaultValue={guard?.conditions?.reserveAmps ?? 5} /><span>A</span></div></label><label className="field">Restore below<div className="input-suffix"><input name="restoreBelowAmps" type="number" min="1" max="400" step="1" required defaultValue={guard?.conditions?.restoreBelowAmps ?? 30} /><span>A</span></div></label><label className="field">Restore delay<div className="input-suffix"><input name="restoreDelaySeconds" type="number" min="0" max="86400" step="30" required defaultValue={guard?.conditions?.restoreDelaySeconds ?? 300} /><span>sec</span></div></label></div><div className="form-footer"><button className="button primary" type="submit" disabled={busy !== null}>{busy === "guard-config" ? "Saving…" : "Save Demand Guard"}</button>{guardResult ? <p className={`inline-save-result ${guardResult.ok ? "success" : "failure"}`} role={guardResult.ok ? "status" : "alert"}>{guardResult.message}</p> : null}</div></form></section> : null}
