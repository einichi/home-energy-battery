...
export function BatteryPage({ view = "status" }: { view?: "status" | "schedules" | "backup" }) {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/set-state-in-effect
    if (currentProfile) setProfile(String(currentProfile));
    const currentReserve = settings?.discharge_limit?.decoded?.percent;
    if (typeof currentReserve === "number") setReserve(currentReserve);
    const currentCharge = settings?.osaifu_charge_window?.decoded;
    if (typeof currentCharge?.start_hour === "number") setChargeStart(currentCharge.start_hour);
    if (typeof currentCharge?.end_hour === "number") setChargeEnd(currentCharge.end_hour);
    const currentDischarge = settings?.osaifu_discharge_window?.decoded;
    if (typeof currentDischarge?.start_hour === "number") setDischargeStart(currentDischarge.start_hour);
    if (typeof currentDischarge?.end_hour === "number") setDischargeEnd(currentDischarge.end_hour);
  }, [status]);
  ...
  ], [chargeStart, chargeEnd, dischargeStart, dischargeEnd, schedules]);
  ...
  return (
    <main className="page battery-page">
      <header className="page-heading">
        <div><p className="eyebrow">Operate</p><h1>{view === "status" ? "Battery" : view === "schedules" ? "Battery schedules" : "Backup Preparation"}</h1><p>{view === "status" ? "Status, deliberate controls, and verified outcomes." : view === "schedules" ? "Plan recurring or one-time changes and review execution results." : "Prepare stored energy for an outage without losing automation ownership."}</p></div>
        <span className="live-state"><i />{loadingState === "ready" ? `Device ${host}` : sentence(loadingState)}</span>
      </header>
      <nav className="battery-navigation" aria-label="Battery sections"><NavLink to="/battery" end>Status & control</NavLink><NavLink to="/battery/schedules">Schedules</NavLink><NavLink to="/battery/backup">Backup preparation</NavLink></nav>
      {backup?.active ? <div className="backup-banner" role="status">Backup Preparation is active · Profile {sentence(backup.currentProfile)}</div> : null}
      ...
      <div className="battery-workspace">
        <section className="panel control-panel">
          ...
          <label className="field"><span>Charging profile</span><select value={profile} onChange={(event) => setProfile(event.target.value)}><option value="eco">Eco</option><option value="osaifu">Osaifu</option><option value="backup">Backup</option></select></label>
          <button className="button" type="button" onClick={() => openReview({ action: "vendor-profile", label: `Set ${sentence(profile)} profile`, payload: { mode: profile }, impact: "Changing profile can alter when the battery charges and how much energy it keeps available." })}>Review profile change</button>
          <label className="field"><span>Minimum reserve</span><select value={reserve} onChange={(event) => setReserve(Number(event.target.value))}>{Array.from({ length: 11 }, (_, step) => <option key={step} value={step * 10}>{step * 10}%</option>)}</select></label>
          <button className="button" type="button" onClick={() => openReview({ action: "discharge-limit", label: `Set reserve to ${reserve}%`, payload: { percent: reserve }, impact: "The battery will avoid discharging below this limit when the device supports the setting." })}>Review reserve change</button>
          <details className="advanced-controls"><summary>Osaifu time windows</summary>
            <div className="window-row"><span>Charge</span><select aria-label="Charge start" value={chargeStart} onChange={(event) => setChargeStart(Number(event.target.value))}>{hourOptions()}</select><span>to</span><select aria-label="Charge end" value={chargeEnd} onChange={(event) => setChargeEnd(Number(event.target.value))}>{hourOptions()}</select><button className="quiet-button" type="button" onClick={() => openReview({ action: "osaifu-charge-window", label: "Change charge window", payload: { startHour: chargeStart, endHour: chargeEnd }, impact: "This changes the battery’s configured Osaifu charging hours." })}>Review</button></div>
            <div className="window-row"><span>Discharge</span><select aria-label="Discharge start" value={dischargeStart} onChange={(event) => setDischargeStart(Number(event.target.value))}>{hourOptions()}</select><span>to</span><select aria-label="Discharge end" value={dischargeEnd} onChange={(event) => setDischargeEnd(Number(event.target.value))}>{hourOptions()}</select><button className="quiet-button" type="button" onClick={() => openReview({ action: "osaifu-discharge-window", label: "Change discharge window", payload: { startHour: dischargeStart, endHour: dischargeEnd }, impact: "This changes the battery’s configured Osaifu discharging hours." })}>Review</button></div>
          </details>
        </section>

        <details className="panel control-panel manual-panel">
          <summary className="manual-summary"><span><span className="eyebrow">Manual control</span><strong>Direct operation</strong><small>Charge, discharge, Standby, or return to Auto</small></span></summary>
      ...
      <section className="panel backup-panel">
        <div><p className="eyebrow">Operational mode</p><h2>Backup Preparation</h2><p>Hold the backup charging profile while coordinating Adaptive Charging and Demand Guard ownership.</p></div>
        ...
        <button className={`button${backup?.active ? " secondary" : " primary"}`} type="button" onClick={() => openReview({ action: backup?.active ? "backup-end" : "backup-start", label: backup?.active ? "End Backup Preparation" : "Start Backup Preparation", payload: { allowDemandGuard: backupAllowDemandGuard, reserve }, impact: backup?.active ? "The prior charging profile will be restored and Adaptive Charging will recalculate before resuming." : `The backup profile will be held. Reserve remains ${reserve}%. Adaptive Charging will pause; Demand Guard will ${backupAllowDemandGuard ? "remain available" : "be released"}. Ending restores the prior profile and recalculates automation.` })}>{backup?.active ? "Review end" : "Review start"}</button>
      </section> : null}
      ...
      <section className="panel schedules-panel">
        ...
        <ScheduleCalendar schedules={schedules} conflicts={conflictingScheduleIds} />
        ...
        <div className="schedule-list">
          {schedules.map((schedule) => { const next = scheduleNextAt(schedule); return <article key={schedule.id}><div><strong>{schedule.name}</strong><span>{actionLabels[schedule.action] ?? sentence(schedule.action)} · {scheduleRecurrence(schedule)}</span><small>Next run: {next ? next.toLocaleString() : schedule.enabled ? "No future occurrence" : "Disabled"}</small>{schedule.lastResult ? <small data-ok={schedule.lastResult.ok}>{schedule.lastResult.ok ? `Last run succeeded${schedule.lastResult.at ? ` · ${new Date(schedule.lastResult.at).toLocaleString()}` : ""}` : `Last run failed: ${schedule.lastResult.error}`}</small> : null}{conflictingScheduleIds.has(schedule.id) ? <p className="schedule-conflict">Conflicts with another enabled schedule at this time.</p> : null}</div><div className="button-row"><button className="quiet-button" type="button" onClick={() => void updateSchedule(schedule.id, { enabled: !schedule.enabled }).then(loadOperations)}>{schedule.enabled ? "Disable" : "Enable"}</button><button className="quiet-button danger" type="button" onClick={() => void deleteSchedule(schedule.id).then(loadOperations)}>Delete</button></div></article>; })}
      ...
      <section className="panel receipts-panel">
        ...
        <div className="receipt-list">
          {receipts.map((receipt) => <article key={receipt.commandId}><i data-state={receipt.state} /><div><strong>{actionLabels[receipt.action] ?? sentence(receipt.action)}</strong><span>{requestSummary(receipt)} · {sentence(receipt.source)}</span><small>{receipt.completedAt || receipt.requestedAt ? new Date(receipt.completedAt ?? receipt.requestedAt ?? "").toLocaleString() : "Time unavailable"}{receipt.durationMs !== null && receipt.durationMs !== undefined ? ` · ${receipt.durationMs} ms` : ""}</small>{receipt.error ? <p className="receipt-error">{receipt.error}</p> : null}</div><b data-state={receipt.state}>{sentence(receipt.state)}</b></article>)}
          {!receipts.length ? <p className="empty-copy">No device commands have been recorded yet.</p> : null}
        </div>
      </section>
