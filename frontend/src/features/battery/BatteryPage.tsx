      ...
      <section className="command-dialog panel" role="dialog" aria-modal="true" aria-labelledby="command-title">
        ...
        {phase === "confirming" ? (
          ...
          <>
            ...
            {phase === "mismatched" ? <p className="command-result failure" role="alert"><T text={"The device acknowledged the request, but readback did not match. "} />{state.phase === "mismatched" ? state.actual : null}</p> : null}
    ...
    <main className="page battery-page">
      ...
      <section className="battery-hero panel">
        ...
        <dl className="battery-state-grid">
          ...
          <div><dt><T text={"Latest contact"} /></dt><dd>{status?.read_at ? new Date(status.read_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" }) : "Unavailable"}</dd></div>
        </dl>
      </section>

      <section className="panel strategy-panel">
        <div><p className="eyebrow"><T text={"Current strategy"} /></p><h2>{status?.batteryStrategy?.title ?? "Determining ownership"}</h2><p>{status?.batteryStrategy?.description ?? "Waiting for current automation and override state."}</p></div>
        {status?.batteryStrategy?.manualOverride?.active ? <strong>{status.batteryStrategy.manualOverride.until ? `Until ${new Date(status.batteryStrategy.manualOverride.until).toLocaleString()}` : "Until changed"}</strong> : <span><T text={"No manual override"} /></span>}
      </section>
      ...
      <section className="panel backup-panel">
        ...
        <div className="backup-state"><strong>{text(backup?.active ? "Active" : "Inactive")}</strong><span>{backup?.active ? text("Since {value}", { value: backup.startedAt ? new Date(backup.startedAt).toLocaleString() : text("recently") }) : text("Normal automation can operate")}</span></div>
