...
function scheduleRecurrence(schedule: BatterySchedule) {
  if (schedule.repeat === "once") return `Once · ${formatDateTime(schedule.runAt)}`;
...
export function BatteryPage({ view = "status" }: { view?: "status" | "schedules" | "backup" }) {
  ...
  const saveEverydayControls = async () => {
    ...
    try {
      ...
      setEverydayResult({ ok: false, message: formatDateTimesInText(reason instanceof Error ? reason.message : "The changes could not be verified.") });
      await loadOperations();
    } finally {
