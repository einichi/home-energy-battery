              ...
              : url.endsWith("/api/discovery/jobs") && init?.method === "POST"
                ...
                : url.endsWith("/api/discovery/jobs/discovery-1")
                  ...
                  : url.endsWith("/api/config")
                    ...
                    : url.includes("/api/history/summary?")
                      ? history.summary
...
describe("React application shell", () => {
  it("renders the next charging window as a localized human-readable range", async () => {
    ...
    fetchMock.mockImplementation((input: RequestInfo | URL, init?: RequestInit) => {
      if (String(input).endsWith("/api/status")) {
        return Promise.resolve({ ok: true, status: 200, json: async () => ({
          ...status,
          batteryStrategy: {
            ...status.batteryStrategy,
            nextAction: {
              action: "charge",
              title: "Charge",
              reason: "Charging is planned during a discounted period.",
              at: "2026-09-20T18:59:48.945Z",
    ...
    const heading = await screen.findByRole("heading", { name: /Charge .*–/ });
    expect(heading).not.toHaveTextContent("T18:59:48.945Z");
    expect(screen.queryByText(/2026-09-20T18:59:48.945Z/)).not.toBeInTheDocument();
  });

  it("never exposes ISO timestamps embedded in status prose", async () => {
    const fetchMock = mockApi();
    const originalFetch = fetchMock.getMockImplementation()!;
    fetchMock.mockImplementation((input: RequestInfo | URL, init?: RequestInit) => {
      if (String(input).endsWith("/api/status")) {
        return Promise.resolve({ ok: true, status: 200, json: async () => ({
          ...
          batteryStrategy: {
            ...status.batteryStrategy,
            kind: "away",
            description: "Away assumptions remain active until 2026-09-27T07:00:00.000Z.",
          },
        }) } as Response);
      }
      return originalFetch(input, init);
    ...
    expect(await screen.findAllByText(/Away assumptions remain active until/)).not.toHaveLength(0);
    expect(screen.queryByText(/2026-09-27T07:00:00.000Z/)).not.toBeInTheDocument();
  ...
  it("consolidates historical analysis into Reports with one-day custom investigation", async () => {
    const fetchMock = mockApi();
    render(
    ...
    expect(screen.queryByRole("heading", { name: "Estimated savings breakdown" })).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Costs & savings" }));
    expect(await screen.findByText("¥2,110")).toBeVisible();
    ...
    await waitFor(() => expect(fetchMock.mock.calls.some(([input]) => {
      const url = String(input);
      return url.includes("/api/reports/energy?") && url.includes("2026-09-10");
    })).toBe(true));
  });
  ...
  it("renders a simulator banner and read-only overview from the API", async () => {
    ...
    const latestContact = screen.getByText("Latest contact").nextElementSibling;
    expect(latestContact).toHaveTextContent(/\d{2}:\d{2}/);
    expect(latestContact).not.toHaveTextContent("2026");
    ...
    expect(screen.getByRole("img", { name: /Today energy history/ })).toBeVisible();
  ...
  it("does not treat a low Ene-Farm hot-water level as an attention item", async () => {
    const fetchMock = mockApi();
    const originalFetch = fetchMock.getMockImplementation()!;
    fetchMock.mockImplementation((input: RequestInfo | URL, init?: RequestInit) => {
      if (String(input).endsWith("/api/status")) {
        return Promise.resolve({ ok: true, status: 200, json: async () => ({
          ...
          energy: {
            ...status.energy,
            fuel_cells: status.energy.fuel_cells.map((fuelCell) => ({
              ...fuelCell,
    ...
    expect(screen.queryByText("Ene-Farm hot-water level is low")).not.toBeInTheDocument();
  });

  it("consolidates current and calendar-today energy detail on Overview", async () => {
    mockApi();
    render(
      <MemoryRouter initialEntries={["/"]}>
        <AppProviders>
    ...
    expect(await screen.findByRole("heading", { name: "Home energy overview" })).toBeVisible();
    expect(await screen.findByRole("img", { name: /Today energy history/ })).toBeVisible();
    ...
    expect(screen.getByRole("heading", { name: "Kitchen", level: 3 })).toBeVisible();
    ...
    const officeRow = screen.getAllByText("Office").find((element) => element.closest("tr"))!.closest("tr")!;
    expect(officeRow).toHaveAttribute("tabindex", "0");
    fireEvent.click(officeRow.querySelector("td")!);
    expect(officeRow).toHaveAttribute("aria-selected", "true");
    expect(screen.getByRole("heading", { name: "Office", level: 3 })).toBeVisible();
    expect(screen.getByRole("img", { name: "Office circuit power history" })).toBeVisible();
    const kitchenRow = screen.getAllByText("Kitchen").find((element) => element.closest("tr"))!.closest("tr")!;
    fireEvent.keyDown(kitchenRow, { key: "Enter" });
    expect(screen.getByRole("heading", { name: "Kitchen", level: 3 })).toBeVisible();
  ...
  it("applies a Reports period to raw history and aggregated comparisons", async () => {
    const fetchMock = mockApi();
    render(
      <MemoryRouter initialEntries={["/reports"]}>
        <AppProviders>
    ...
    let deferNextHistory = true;
    fetchMock.mockImplementation((input: RequestInfo | URL, init?: RequestInit) => {
      if (deferNextHistory && String(input).includes("/api/history?")) {
    ...
    expect(document.querySelector(".period-results")).toHaveAttribute("aria-busy", "true");
    await waitFor(() => expect(fetchMock.mock.calls.map(([input]) => String(input)).filter((url) => url.includes("/api/history?")).length).toBeGreaterThan(1));
    const historyRequest = fetchMock.mock.calls.map(([input]) => String(input)).findLast((url) => url.includes("/api/history?"));
  ...
  it("restores source composition, Ene-Farm activity, and three off-peak savings views on Overview", async () => {
    ...
    expect(screen.getAllByText("Today", { exact: true }).length).toBeGreaterThan(0);
    expect(screen.getByRole("heading", { name: "Ene-Farm Activity" })).toBeVisible();
    expect(
  ...
  it("uses consistent Overview visibility names and direct mobile destinations", async () => {
    mockApi();
    render(
    ...
    expect(screen.getAllByRole("link", { name: /Settings|System/ }).length).toBeGreaterThan(0);
  ...
  it("keeps five direct destinations in phone navigation", async () => {
    ...
    expect(screen.getAllByRole("navigation", { name: "Primary" }).at(-1)?.querySelectorAll("a")).toHaveLength(5);
    const mobileNav = screen.getAllByRole("navigation", { name: "Primary" }).at(-1)!;
    expect(mobileNav.querySelector('a[href="/"]')).toHaveTextContent("Home");
    expect(mobileNav.querySelector('a[href="/reports"]')).toHaveTextContent("Reports");
    expect(mobileNav.querySelector('a[href="/system"]')).toHaveTextContent("Settings");
  });

  it("provides focused Ene-Farm historical reporting", async () => {
    mockApi();
    render(
      <MemoryRouter initialEntries={["/reports"]}>
        <AppProviders><App /></AppProviders>
    ...
    expect(await screen.findByRole("heading", { name: "Reports", level: 1 })).toBeVisible();
    fireEvent.click(screen.getByRole("button", { name: "Ene-Farm" }));
    expect(await screen.findByRole("heading", { name: "Ene-Farm Activity" })).toBeVisible();
    expect(screen.getByRole("heading", { name: "Ene-Farm detail" })).toBeVisible();
    expect(screen.queryByRole("heading", { name: "Power flows and storage" })).not.toBeInTheDocument();
  });

  it("keeps slow daily summaries in a loading state instead of showing false zeroes", async () => {
    mockApi();
    const baseFetch = globalThis.fetch;
    let releaseSummary!: () => void;
    const summaryGate = new Promise<void>((resolve) => { releaseSummary = resolve; });
    vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      if (String(input).includes("/api/history?")) await summaryGate;
    ...
    expect(await screen.findByText("Loading source composition…")).toBeVisible();
    expect(screen.getByText("Loading today's energy outcomes…")).toBeVisible();
    expect(screen.queryByText("Est. solar saving", { selector: "dt" })).not.toBeInTheDocument();
    releaseSummary();
    expect(await screen.findByText("Est. solar saving", { selector: "dt" })).toBeVisible();
  });
});
