...
The completed mobile information architecture has five primary destinations:
Home, Energy, Battery, Automation, and More. More provides the phone entry points
for Insights and System; desktop retains the full left rail. Ene-Farm detail has a
stable `/ui/energy/ene-farm` route while Overview and Energy retain concise summary
views.
...
The final hardening pass adds runtime API response validation, a compact
`/api/history/summary` view model, interval-resolution chart responses for ranges
over two hours, source-labelled global alerts, last-known/stale reading treatment,
conditional Overview attention, print styles, adverse-state visual fixtures,
automated landmark/control-name accessibility checks, and a repeated-navigation
heap/polling lifecycle gate. These are API/view and test changes only; schema v7 is
unchanged.

Energy has one workspace-level analysis-period control outside the Combined History
card. Its range is the source of truth for history, source/destination balances,
outcomes, battery balance, Ene-Farm, and Smart Cosmo circuits. Period changes expose
an announced busy state and dim prior-period results until both history and Ene-Farm
requests settle; failed requests clear mismatched prior-period values. Circuit labels
remain read-only until **Edit labels** is selected. Edit mode snapshots the displayed
row order, disables sorting, and preserves that order until the batch is saved or
cancelled.
