...
type ProgressCallback = (progress: DatabaseProgress) => void;
type ByteProgressCallback = (progress: { processedBytes: number; totalBytes: number }) => void;
type BackupOptions = {
  databaseFile: string;
  backupDir: string;
...
function progressTransform(totalBytes: number, onProgress?: ByteProgressCallback): Transform {
...
async function cleanupStaleBackupFiles(backupDir: string): Promise<void> {
  let entries: string[] = [];
  try {
...
async function cleanupSnapshotFiles(snapshotFile: string): Promise<void> {
  await Promise.all([
    snapshotFile,
    `${snapshotFile}-wal`,
    `${snapshotFile}-shm`,
...
async function cleanupStaleRestoreExtractions(workingDir: string): Promise<void> {
  let entries: string[] = [];
  try {
...
export function manualDatabaseBackupFilename(sourceVersion: number, now = new Date()): string {
  return `history-v${sourceVersion}-manual-${timestampForFilename(now)}.sqlite.zst`;
}

export function preRestoreDatabaseBackupFilename(sourceVersion: number, now = new Date()): string {
...
export function databaseBackupMetadata(filename: string, currentVersion: number | null = null) {
...
async function createDatabaseBackup({
  ...
  baseFilename,
  onProgress = () => {},
}: BackupOptions & { baseFilename: string }) {
  ...
  if (availableBytes < requiredBytes) {
    const error = new Error(`Insufficient backup space: ${requiredBytes} bytes required, ${availableBytes} bytes available`) as Error & {
      code: string;
...
export async function listDatabaseBackups({ backupDir, currentVersion }: { backupDir: string; currentVersion: number }) {
  await mkdir(backupDir, { recursive: true });
  const backups: Array<ReturnType<typeof databaseBackupMetadata> & { sizeBytes: number; modifiedAt: string }> = [];
...
export async function extractAndValidateDatabaseBackup({
  ...
  workingDir,
  onProgress = () => {},
}: { backupFile: string; workingDir: string; onProgress?: ProgressCallback }) {
  ...
  try {
    ...
    try {
      ...
      const row = metadataTable
        ...
        : null;
      const schemaVersion = row ? JSON.parse(String(row.value)) as unknown : null;
...
export async function deleteDatabaseBackup({ backupDir, filename }: { backupDir: string; filename: string }) {
...
export async function cleanupExtractedDatabaseBackup(snapshotFile: string): Promise<void> {
  await cleanupSnapshotFiles(snapshotFile);
}
