type CounterPolicy = {
  maximum?: number;
  maximumDelta?: number;
  maximumRatePerHour?: number;
  resolution?: number;
};

export type CounterDeltaResult = {
  delta: number | null;
  issue: "rollover" | "reset" | "invalid-jump" | null;
};

function finiteCounterValue(value: unknown): number | null {
...
export function cumulativeCounterDeltaResult(
  current: unknown,
  previous: unknown,
  policy: CounterPolicy = {},
  elapsedSeconds: unknown = null,
): CounterDeltaResult {
  ...
  let delta: number;
  let issue: CounterDeltaResult["issue"] = null;
  if (now >= before) {
    delta = now - before;
...
export function finiteCounterMap(channels: Array<{ channel?: unknown; value?: unknown }> = []): Record<string, number> {
