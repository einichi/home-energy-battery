...
export function normalizeRetentionPolicy(policy = {}, legacyRawTelemetryDays = undefined) {
  ...
  return {
    ...
    plannerHistoryDays: days(policy.plannerHistoryDays, null),
...
export function createHistoryStore({ dataDir, logger = console } = {}) {
  ...
  const legacyForecastFile = path.join(dataDir, "solar-planner", "forecast-snapshots.jsonl");
  const legacyWeatherFile = path.join(dataDir, "solar-planner", "historical-weather.jsonl");
  let database = null;
  ...
  async function migratePlannerContext() {
  ...
  async function applyRetention(policyInput = {}, now = new Date()) {
    ...
    const eventRetention = [
      ["planner", policy.plannerHistoryDays, "plannerEvents"],
    ...
    if (policy.plannerHistoryDays !== null) {
      ...
      `, [cutoff(policy.plannerHistoryDays)]);
      await deleteInChunks(`
