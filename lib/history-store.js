...
export function createHistoryStore({
  ...
  function batteryChargeCurveSamples(sessions = []) {
  ...
  function recentEvents(category, limit = 50, beforeMs = Date.now()) {
    ...
    return requireDatabase().prepare(`
      ...
      LIMIT ?
    `).all(category, normalizedBeforeMs, normalizedLimit).map((row) => ({
  ...
  function eventsByKeyPrefix(prefix) {
    if (!prefix) return [];
    return requireDatabase().prepare(`
      SELECT event_key, timestamp, type, message, payload_json FROM events
