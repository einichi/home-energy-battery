...
export const ENERGY_CALCULATION_VERSION = 2;
...
function rollupPayload(state) {
  ...
  for (const [key, metric] of Object.entries(state.powers)) {
    if (metric.weight > 0) payload[key] = metric.weightedSum / metric.weight;
    else if (metric.count > 0) payload[key] = metric.sum / metric.count;
  }
...
export function createHistoryStore({ dataDir, logger = console } = {}) {
  ...
  function insertSample(sample, { sourceFile = null, sourceLine = null } = {}) {
    ...
    const enriched = enrichHistorySample(sample, previousSample, { metricBaselines });
