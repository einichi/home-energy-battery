...
function finite(value: any): any {
  if (value === null || value === undefined || value === "" || typeof value === "boolean") return null;
  const number = Number(value);
  return Number.isFinite(number) ? number : null;
}

function timestampMs(value: any): any {
...
function localBucketStart(timeMs: any, resolution: any): any {
...
function bucketEnd(startMs: any, resolution: any): any {
  if (resolution === "daily") {
...
function maximumIntegrationGapMs(sample: any): any {
...
function intervalEnergyDetails(sample: any, previousSample: any, directKey: any, wattsKey: any, transform: any = (value: any) => Math.max(0, value)): any {
...
function intervalEnergy(sample: any, previousSample: any, directKey: any, wattsKey: any, transform: any = (value: any) => Math.max(0, value)): any {
  return intervalEnergyDetails(sample, previousSample, directKey, wattsKey, transform)?.value ?? null;
}

function baselineForMetric(previousSample: any, metricBaselines: any, key: any): any {
  return metricBaselines?.[key] ?? previousSample;
}

function updateMetricBaselines(metricBaselines: any, sample: any): any {
...
export function compactHistorySample(sample: any = {}): any {
  const compact: Record<string, any> = { ...sample };
  for (const key of DERIVED_SAMPLE_KEYS) delete compact[key];
  return compact;
}

export function enrichHistorySample(sample: any, previousSample: any = null, { metricBaselines = null }: any = {}): any {
  const enriched: Record<string, any> = { ...sample, calculationVersion: ENERGY_CALCULATION_VERSION };
  const mappings: any[] = [
    ["houseDemandKwh", "houseDemandW", (value: any) => Math.max(0, value)],
    ["solarGenerationKwh", "solarPowerW", (value: any) => Math.max(0, value)],
    ["gridImportKwh", "gridImportW", (value: any) => Math.max(0, value)],
    ["gridExportKwh", "gridExportW", (value: any) => Math.max(0, value)],
    ["fuelCellKwh", "fuelCellPowerW", (value: any) => Math.max(0, value)],
    ["batteryChargeKwh", "batteryPowerW", (value: any) => Math.max(0, value)],
    ["batteryDischargeKwh", "batteryPowerW", (value: any) => Math.max(0, -value)],
  ];
  const coverageSeconds: Record<string, any> = { ...(sample?.coverageSeconds ?? {}) };
  const energyQuality: Record<string, any> = { ...(sample?.energyQuality ?? {}) };
  const powerCoverageSeconds: Record<string, any> = { ...(sample?.powerCoverageSeconds ?? {}) };
  const intervalAveragePowerW: Record<string, any> = { ...(sample?.intervalAveragePowerW ?? {}) };
  const intervalAverageCircuitPowerW: Record<string, any> = { ...(sample?.intervalAverageCircuitPowerW ?? {}) };
  const energyIntervalStart: Record<string, any> = { ...(sample?.energyIntervalStart ?? {}) };
  ...
  const circuitEnergyKwh: Record<string, any> = { ...(sample?.circuitEnergyKwh ?? {}) };
...
export function interpretHistorySample(rawSample: any, previousRawSample: any = null, { metricBaselines = null }: any = {}): any {
  ...
  const unavailableCounter: Record<string, any> = { delta: null, issue: null };
  ...
  const currentCircuits = Object.fromEntries(Object.entries(sample.circuitCumulativeKwh ?? {})
    .map(([channel, value]: any) => [channel, finite(value)])
  ...
  const exactHouseDemandKwh = Object.keys(currentCircuits).length > 0
    ...
    ? Object.values(circuitEnergyKwh).reduce((sum: any, value: any) => sum + value, 0)
    : null;

  const interpreted: Record<string, any> = { ...sample };
  const directMetrics: any[] = [
...
function emptyRollupState(startMs: any, resolution: any): any {
  return {
    resolution,
    startMs,
...
function addAverageMetric(target: any, key: any, value: any, weight: any = 1): any {
...
function addRollupSample(state: any, sample: any): any {
...
function rollupPayload(state: any): any {
  const payload: Record<string, any> = {
    ...
    energyQuality: Object.fromEntries(Object.entries(state.energyQualities ?? {}).map(([key, counts]: any) => {
      const qualities = Object.keys(counts).filter((quality: any) => Number(counts[quality]) > 0);
  ...
  const powerCoverageSeconds: Record<string, any> = {};
  const intervalAveragePowerW: Record<string, any> = {};
  for (const [key, metric] of Object.entries(state.powers) as Array<[string, any]>) {
    if (metric.weight > 0) {
  ...
  for (const [key, energy] of Object.entries(state.energy) as Array<[string, any]>) {
    if (energy.count > 0) payload[key] = energy.sum;
  }
  const circuitPowerW: Record<string, any> = {};
  for (const [channel, metric] of Object.entries(state.circuits.power) as Array<[string, any]>) {
...
function parseJson(text: any, fallback: any = null): any {
  try {
...
export function normalizeRetentionPolicy(policy: any = {}, legacyRawTelemetryDays: any = undefined): any {
  const days = (value: any, fallback: any) => {
...
export function createHistoryStore({
  ...
  let previousSample: any = null;
  let metricBaselines: Record<string, any> = {};
  ...
  function metadataGet(key: any, fallback: any = null): any {
    const row = requireDatabase().prepare("SELECT value FROM metadata WHERE key = ?").get(key);
    return row ? parseJson(row.value, fallback) : fallback;
  }

  function metadataSet(key: any, value: any): any {
  ...
  function createSchema(): any {
    ...
    metadataSet("architectureVersion", ARCHITECTURE_VERSION);
  }

  function loadRollupState(resolution: any, startMs: any): any {
  ...
  function persistRollup(state: any): any {
  ...
  function updateRollups(sample: any): any {
  ...
  function insertSample(sample: any, { sourceFile = null, sourceLine = null }: any = {}): any {
    const timeMs = timestampMs(sample?.timestamp);
  ...
  function appendSample(sample: any): any {
    const db = requireDatabase();
  ...
  function recentMetricBaselines(): any {
  ...
  function recordForecast(forecast: any): any {
  ...
  function recordSolarForecastIssues(issues: any = []): any {
    ...
    for (const issue of issues) {
      ...
      if (!issue?.targetDate || issuedAtMs === null || periodStartMs === null || periodEndMs === null
        || periodEndMs <= periodStartMs || values.some((value: any) => value === null)) continue;
      statement.run(
        issue.targetDate,
        issuedAtMs,
        issuedAt,
  ...
  function settleSolarForecastOutcomes(now: any = new Date()): any {
  ...
  function canonicalSolarForecastRows({ completedOnly = true }: any = {}): any {
    ...
    return [...grouped.values()].map((candidates: any) => {
      const beforeStart = candidates.filter((row: any) => row.issued_at_ms <= row.period_start_ms);
      return beforeStart.length ? beforeStart.at(-1) : candidates[0];
    });
  }

  function solarForecastOutcomes(limit: any = 30): any {
    const rows = canonicalSolarForecastRows();
    return rows.slice(-Math.max(1, Math.round(limit))).map((row: any) => {
  ...
  function solarForecastAccuracy(limit: any = 30): any {
    const outcomes = solarForecastOutcomes(limit);
    const ratios = outcomes
      .filter((outcome: any) => outcome.rawPredictedKwh > 0.05 && outcome.actualKwh >= 0)
      .map((outcome: any) => outcome.actualKwh / outcome.rawPredictedKwh)
      .filter(Number.isFinite);
    const measuredFactor = median(ratios);
    const learned = ratios.length >= 5 && Number.isFinite(measuredFactor);
    const errors = outcomes
      .map((outcome: any) => outcome.errorPercent)
      .filter(Number.isFinite)
      .map(Math.abs);
    return {
      learned,
      ...
      meanAbsolutePercentageError: errors.length
        ? errors.reduce((sum: any, value: any) => sum + value, 0) / errors.length
        : null,
      outcomes,
    };
  }

  function insertWeather(records: any = []): any {
    const statement = requireDatabase().prepare(`
  ...
  function recordWeather(records: any = []): any {
    const db = requireDatabase();
    db.exec("BEGIN IMMEDIATE");
    try {
  ...
  function recordFuelCellForecasts(entries: any = [], issuedAt: any = new Date().toISOString()): any {
    const issuedAtMs = timestampMs(issuedAt);
  ...
  function settleFuelCellForecastOutcomes(now: any = new Date()): any {
    ...
    for (const row of rows) {
      ...
      `).all(row.target_start_ms, endMs).map((sample: any) => parseJson(sample.payload_json, {}));
      let actualKwh = 0;
      let measured = false;
  ...
  function fuelCellForecastOutcomes(limit: any = 100): any {
    ...
    `).all(Math.max(1, Math.round(limit))).map((row: any) => ({
  ...
  function gasTariffSnapshots({ provider = "tokyo-gas", billingMonth = null }: any = {}): any {
  ...
  function recordGasTariffSnapshot(snapshot: any): any {
  ...
  function gasTariffOverride(provider: any = "tokyo-gas", billingMonth: any): any {
    const row = requireDatabase().prepare(`
  ...
  function setGasTariffOverride(provider: any, billingMonth: any, payload: any): any {
  ...
  function deleteGasTariffOverride(provider: any, billingMonth: any): any {
  ...
  function interpretedRawPayloads(startMs: any, endMs: any): any {
  ...
  function rollupRowsToPayloads(rows: any): any {
    return rows.map((row: any) => {
      const payload = parseJson(row.payload_json);
      if (!payload) return null;
      const state = parseJson(row.state_json, {});
      const powerCoverageSeconds: Record<string, any> = { ...(payload.powerCoverageSeconds ?? {}) };
      const intervalAveragePowerW: Record<string, any> = { ...(payload.intervalAveragePowerW ?? {}) };
      for (const [key, metric] of Object.entries(state.powers ?? {}) as Array<[string, any]>) {
  ...
  function rawRows(startMs: any, endMs: any): any {
    return requireDatabase().prepare(`
      SELECT payload_json FROM samples
  ...
  function rollupRows(startMs: any, endMs: any, resolution: any): any {
    return requireDatabase().prepare(`
  ...
  function rawRangeStats(startMs: any, endMs: any): any {
    const row = requireDatabase().prepare(`
  ...
  function rawRangePayloadBytes(startMs: any, endMs: any): any {
    const row = requireDatabase().prepare(`
  ...
  function intervalRangeBounds(startMs: any, endMs: any): any {
  ...
  function detailedAvailableRows(startMs: any, endMs: any, earliestRaw: any): any {
  ...
  function querySamples(startMs: any, endMs: any, { resolution = "auto" }: any = {}): any {
  ...
  function batteryChargeCurveSamples(sessions: any = []): any {
    const query = requireDatabase().prepare(`
    ...
    for (const [sessionIndex, session] of sessions.entries()) {
      ...
      for (const row of query.all(startMs, endMs)) {
  ...
  function recordEvent({ eventKey, at, category, type = "event", message = null, payload = null }: any): any {
  ...
  function eventsBetween(category: any, startMs: any, endMs: any, types: any = []): any {
    if (!Number.isFinite(startMs) || !Number.isFinite(endMs) || startMs > endMs) return [];
    ...
    return requireDatabase().prepare(`
      ...
      ORDER BY timestamp_ms, id
    `).all(category, startMs, endMs, ...normalizedTypes).map((row: any) => ({
      at: row.timestamp,
      type: row.type,
      message: row.message,
  ...
  function recentEvents(category: any, limit: any = 50, beforeMs: any = Date.now()): any {
    ...
    return requireDatabase().prepare(`
      ...
      LIMIT ?
    `).all(category, normalizedBeforeMs, normalizedLimit).map((row: any) => ({
  ...
  function eventsByKeyPrefix(prefix: any): any {
    if (!prefix) return [];
    return requireDatabase().prepare(`
      SELECT event_key, timestamp, type, message, payload_json FROM events
      WHERE event_key LIKE ?
      ORDER BY timestamp_ms, id
    `).all(`${prefix}%`).map((row: any) => ({
      eventKey: row.event_key,
      at: row.timestamp,
      type: row.type,
      message: row.message,
  ...
  function tagEventsBefore(category: any, before: any, fields: any = {}): any {
    ...
    try {
      for (const row of rows) {
        ...
        if (Object.keys(fields).every((key: any) => payload?.[key] !== undefined)) continue;
  ...
  function awayPeriodView(row: any, nowMs: any = Date.now()): any {
    if (!row) return null;
  ...
  function awayPeriod(id: any, nowMs: any = Date.now()): any {
    return awayPeriodView(requireDatabase().prepare(`
      SELECT * FROM away_periods WHERE id = ?
    `).get(id), nowMs);
  }

  function awayPeriods({ includeCompleted = true, startMs = null, endMs = null, nowMs = Date.now() }: any = {}): any {
    const rows = Number.isFinite(startMs) && Number.isFinite(endMs)
      ...
      : requireDatabase().prepare(`
        ...
        `).all();
    return rows
      .map((row: any) => awayPeriodView(row, nowMs))
      .filter((period: any) => includeCompleted || period.status !== "completed");
  }

  function createAwayPeriod(period: any): any {
    requireDatabase().prepare(`
  ...
  function updateAwayPeriod(period: any): any {
    const result = requireDatabase().prepare(`
      UPDATE away_periods
  ...
  function deleteAwayPeriod(id: any): any {
    return Number(requireDatabase().prepare("DELETE FROM away_periods WHERE id = ?").run(id).changes) > 0;
  }

  function historicalWeather(): any {
    return requireDatabase().prepare("SELECT payload_json FROM weather ORDER BY time_ms").all()
      .map((row: any) => parseJson(row.payload_json)).filter(Boolean);
  }

  async function databaseFileSizes(): Promise<any> {
    const size = async (file: any) => {
      try {
  ...
  async function deleteInChunks(sql: any, parameters: any = []): Promise<any> {
  ...
  async function applyRetention(policyInput: any = {}, now: any = new Date()): Promise<any> {
    const policy = normalizeRetentionPolicy(policyInput);
    const before = await stats();
    const cutoff = (days: any) => now.getTime() - days * 86_400_000;
...
export function historyDatabaseFile(dataDir: any): any {
  return path.join(dataDir, "history.sqlite");
}

export async function inspectHistoryDatabase(dataDir: any): Promise<any> {
    ...
    try {
      ...
      if (version !== SCHEMA_VERSION) {
        return {
          ...
          error: `Database schema v${version} is incompatible; schema v${SCHEMA_VERSION} is required. Run the bridge release first.`,
        };
      }
      return {
        state: "current",
