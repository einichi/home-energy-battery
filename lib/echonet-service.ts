...
const DEFAULT_VENDOR_EPCS: any[] = [0xd7, 0xd9, 0xf0, 0xf4, 0xf5, 0xf6];

const EPC: Record<string, any> = {
...
const MODE_TO_EDT: Record<string, any> = {
  rapid: 0x41,
  rapid_charging: 0x41,
  charge: 0x42,
  charging: 0x42,
...
const EDT_TO_MODE: Record<string, any> = {
  0x40: "other",
  0x41: "rapid_charging",
  0x42: "charging",
  0x43: "discharging",
...
const VENDOR_PROFILE_TO_EDT: Record<string, any> = {
  osaifu: 0x02,
  eco: 0x03,
  backup: 0x20,
};

const EDT_TO_VENDOR_PROFILE: Record<string, any> = {
  0x02: "osaifu",
  0x03: "eco",
  0x20: "backup",
};

const EDT_TO_FUEL_CELL_STATUS: Record<string, any> = {
  0x40: "other",
  0x41: "generating",
  0x42: "stopped",
  0x43: "starting",
...
const EDT_TO_FUEL_CELL_INTERCONNECTION: Record<string, any> = {
...
const EOJ_CLASS_NAMES: Record<string, any> = {
...
function parseByte(value: any): any {
...
function parseEoj(value: any): any {
  ...
  return [0, 2, 4].map((i: any) => Number.parseInt(hex.slice(i, i + 2), 16));
}

function eojHex(eoj: any): any {
  return `0x${eoj.map((b: any) => b.toString(16).padStart(2, "0")).join("").toUpperCase()}`;
}

function eojName(eoj: any): any {
  const klass = eoj.slice(0, 2).map((b: any) => b.toString(16).padStart(2, "0")).join("").toUpperCase();
  return `${EOJ_CLASS_NAMES[klass] ?? "unknown"}(${eojHex(eoj)})`;
}

function parseHexBytes(value: any): any {
...
function parseHour(value: any, label: any): any {
...
function formatHour(hour: any): any {
  return `${hour.toString().padStart(2, "0")}:00`;
}

function rawHex(raw: any): any {
  return raw ? `0x${Buffer.from(raw).toString("hex")}` : null;
}

function propRaw(res: any, epc: any): any {
  const prop = res?.message?.prop?.find((p: any) => p.epc === epc);
  if (!prop) return null;
...
function acknowledgedSet(res: any, description: any): any {
...
function decodedOrRawData(res: any): any {
  const data = res?.message?.data;
  if (Buffer.isBuffer(data)) return rawHex(data);
  return data ?? null;
}

function mapToHex(map: any): any {
  if (!Array.isArray(map)) return [];
  return map.map((n: any) => `0x${n.toString(16).padStart(2, "0").toUpperCase()}`);
}

function metric({ host, eoj, epc, name, raw, value = undefined, unit = undefined, human = undefined }: any): any {
  const out: Record<string, any> = { host, eoj, epc: `0x${epc.toString(16).padStart(2, "0").toUpperCase()}`, name, raw: rawHex(raw) };
...
function decodeUnsigned({ host, eoj, epc, name, raw, unit }: any): any {
...
function decodeCumulativeKwh({ host, eoj, epc, name, raw, unit }: any): any {
...
function decodeFuelCellCumulative({ host, epc, name, raw, unit }: any): any {
...
function decodeFuelCellHotWaterLevel({ host, raw }: any): any {
...
function decodeInstantPowerList(raw: any): any {
...
function decodeCumulativePowerList(raw: any, unit: any): any {
...
function sumInstantPowerChannels(decoded: any): any {
...
function decodeEnum({ host, eoj, epc, name, raw, mapping }: any): any {
...
function decodeVendorProfile(raw: any): any {
...
function decodeOsaifuWindow(raw: any): any {
...
function decodeVendorProperty(epc: any, raw: any): any {
  if (!raw || raw.length === 0) return null;
...
class EchonetNode {
  ...
  constructor({ timeout = 3, netif = "", debug = false }: any = {}) {
    const params: Record<string, any> = { type: "lan" };
  ...
  init(): any {
    return new Promise((resolve: any, reject: any) => {
      this.el.init((err: any) => (err ? reject(err) : resolve()));
    });
  }

  close(): any {
    return new Promise((resolve: any) => this.el.close(resolve));
  }

  withTimeout(work: any, label: any): any {
  ...
  get(host: any, eoj: any, epc: any): any {
    return this.withTimeout(
      (cb: any) => this.el.getPropertyValue(host, eoj, epc, cb),
      `${host} ${eojHex(eoj)} EPC 0x${epc.toString(16)}`
    );
  }

  set(host: any, eoj: any, epc: any, edt: any): any {
    return this.withTimeout(
      (cb: any) => this.el.setPropertyValue(host, eoj, epc, edt, cb),
      `${host} ${eojHex(eoj)} EPC 0x${epc.toString(16)} set`
    );
  }

  maps(host: any, eoj: any): any {
    return this.withTimeout((cb: any) => this.el.getPropertyMaps(host, eoj, cb), `${host} ${eojHex(eoj)} maps`);
  }
...
function makeClient(opts: any): any {
...
async function withClient(opts: any, fn: any): Promise<any> {
...
async function cmdDiscover(opts: any): Promise<any> {
  return withClient(opts, async (client: any) => {
    const devices = await client.discover();
    return devices;
  });
}

async function cmdInspectHost(opts: any): Promise<any> {
...
async function cmdProbe(opts: any): Promise<any> {
  const host = required(opts.host, "--host");
  const instance = Number(opts.instance ?? 1);
  return withClient(opts, async (client: any) => {
    const eoj: any[] = [0x02, 0x7d, instance];
    const maps = await client.maps(host, eoj);
    return {
      [eojHex(eoj)]: {
        set_property_map: mapToHex(maps.message.data.set),
...
async function cmdDumpEoj(opts: any): Promise<any> {
  const host = required(opts.host, "--host");
  ...
  return withClient(opts, async (client: any) => {
    ...
    if (opts.epc !== undefined) {
      ...
      const mapProp = maps.message.prop.find((p: any) => p.epc === EPC.GET_PROPERTY_MAP);
    ...
    const out: Record<string, any> = {
      ...
      get_property_map: mapHex.length ? mapHex : epcs.map((epc: any) => `0x${epc.toString(16).padStart(2, "0").toUpperCase()}`),
      properties: {},
    };
...
async function cmdDumpVendor(opts: any): Promise<any> {
  const host = required(opts.host, "--host");
  const eoj = parseEoj(opts.eoj ?? STORAGE_BATTERY_EOJ);
  ...
  return withClient(opts, async (client: any) => {
    ...
    const epcs = requested.filter((epc: any) => !readable.length || readable.includes(epc) || writable.includes(epc));
    const out: Record<string, any> = {
      host,
      eoj: eojHex(eoj),
      name: eojName(eoj),
      vendor_epcs: epcs.map((epc: any) => `0x${epc.toString(16).padStart(2, "0").toUpperCase()}`),
      writable_vendor_epcs: epcs
        .filter((epc: any) => writable.includes(epc))
        .map((epc: any) => `0x${epc.toString(16).padStart(2, "0").toUpperCase()}`),
      properties: {},
    };
...
async function cmdRawGet(opts: any): Promise<any> {
  const host = required(opts.host, "--host");
  const eoj = parseEoj(opts.eoj ?? STORAGE_BATTERY_EOJ);
...
async function cmdRawSet(opts: any): Promise<any> {
  const host = required(opts.host, "--host");
  const eoj = parseEoj(opts.eoj ?? STORAGE_BATTERY_EOJ);
  const epc = parseByte(required(opts._[1], "EPC"));
...
async function cmdStatus(opts: any): Promise<any> {
  const host = required(opts.host, "--host");
  const instance = Number(opts.instance ?? 1);
  const eoj: any[] = [0x02, 0x7d, instance];
  const props: any[] = [
...
async function cmdEnergyStatus(opts: any): Promise<any> {
  ...
  return withClient(opts, async (client: any) => {
    const errors: any[] = [];
    async function read(host: any, eoj: any, epc: any): Promise<any> {
      try {
    ...
    const fuelCellSources = fuelCellEnabled
      ? [
          ...
          ...fuelCellProxyHosts.filter((host: any) => host !== fuelCellPrimaryHost).map((host: any) => ({ host, role: "proxy" })),
        ]
      : [];
...
async function cmdMeterStatus(opts: any): Promise<any> {
  const host = required(opts.host, "--host");
  const eojText = opts.eoj ?? POWER_METER_EOJ;
  const eoj = parseEoj(eojText);
  return withClient(opts, async (client: any) => {
    async function read(epc: any): Promise<any> {
      try {
        const res = await client.get(host, eoj, epc);
        return propRaw(res, epc);
      } catch (err: any) {
...
async function setOperationMode(opts: any, mode: any): Promise<any> {
  const host = required(opts.host, "--host");
  const instance = Number(opts.instance ?? 1);
...
async function cmdSetMode(opts: any): Promise<any> {
  return setOperationMode(opts, required(opts._[1], "MODE"));
}

async function cmdVendorProfile(opts: any): Promise<any> {
  const host = required(opts.host, "--host");
  const instance = Number(opts.instance ?? 1);
  const eoj: any[] = [0x02, 0x7d, instance];
...
async function cmdOsaifuWindow(opts: any, kind: any): Promise<any> {
  const host = required(opts.host, "--host");
  const instance = Number(opts.instance ?? 1);
  const eoj: any[] = [0x02, 0x7d, instance];
...
async function cmdDischargeLimit(opts: any): Promise<any> {
  const host = required(opts.host, "--host");
  const instance = Number(opts.instance ?? 1);
  const eoj: any[] = [0x02, 0x7d, instance];
...
async function cmdChargeLike(opts: any, mode: any): Promise<any> {
  const host = required(opts.host, "--host");
  const instance = Number(opts.instance ?? 1);
  const eoj: any[] = [0x02, 0x7d, instance];
  const writes: any[] = [];
  ...
  if (opts["dry-run"]) {
    return { host, eoj: eojHex(eoj), writes: writes.map((w: any) => ({ epc: `0x${w.epc.toString(16)}`, edt: rawHex(w.edt) })) };
  }
  return withClient(opts, async (client: any) => {
    const results: any[] = [];
...
function required(value: any, label: any): any {
  if (value === undefined || value === "") throw new Error(`${label} is required`);
  return value;
}

function values(value: any, fallback: any): any {
  if (value === undefined) return fallback;
  return Array.isArray(value) ? value : [value];
}

const COMMAND_HANDLERS: Record<string, any> = {
  ...
  "osaifu-charge-window": (opts: any) => cmdOsaifuWindow(opts, "charge"),
  "osaifu-discharge-window": (opts: any) => cmdOsaifuWindow(opts, "discharge"),
  "discharge-limit": cmdDischargeLimit,
  charge: (opts: any) => cmdChargeLike(opts, "charge"),
  discharge: (opts: any) => cmdChargeLike(opts, "discharge"),
};

async function executeEchonetCommand(command: any, args: any = {}, positional: any = [], client: any = null): Promise<any> {
...
async function createEchonetCommandAdapter(options: any = {}): Promise<any> {
  const client = options.client ?? makeClient(options);
  let initialization: any = null;
  let closed = false;
  return {
    async execute(command: any, args: any = {}, positional: any = []): Promise<any> {
