...
export type GasTariffBand = {
  minM3: number;
  maxM3: number | null;
  baseChargeYen: number;
  yenPerM3: number;
  label: string;
};
export type GasTariffDiscount = {
  id: string;
  label: string;
  seasons: string[];
  percent: number;
...
function finite<T extends number | null>(value: unknown, fallback: T = null as T): number | T {
  const number = Number(value);
  return Number.isFinite(number) ? number : fallback;
}

export function validBillingMonth(value: unknown): value is string {
  return /^\d{4}-(0[1-9]|1[0-2])$/.test(String(value ?? ""));
}

export function normalizeGasTariffPayload(value: JsonRecord = {}): GasTariff {
  const bands = (Array.isArray(value.bands) ? value.bands : [])
    ...
    .filter((band: GasTariffBand) => band.maxM3 === null || band.maxM3 > band.minM3)
  ...
  const discounts = (Array.isArray(value.discounts) ? value.discounts : [])
    ...
    .filter((discount: GasTariffDiscount) => discount.id && discount.percent > 0);
  return {
...
export function gasTariffHash(payload: unknown): string {
  return createHash("sha256").update(JSON.stringify(payload)).digest("hex");
}

export function tokyoGasSeasonForBillingMonth(billingMonth: string): "winter" | "other" {
...
function tokyoGasRegion(region = "tokyo") {
  return TOKYO_GAS_REGIONS[region as keyof typeof TOKYO_GAS_REGIONS] ?? TOKYO_GAS_REGIONS.tokyo;
}

export function tokyoGasTariffUrl(billingMonth: string, readingDay = 1, region = "tokyo"): string {
...
function textRows(html: unknown): string[] {
  return String(html)
...
export function parseTokyoGasTariffHtml(html: unknown, { season = "other" }: { season?: string } = {}): GasTariff {
  const bands: GasTariffBand[] = [];
...
function tokyoGasDiscounts(season: "winter" | "other"): GasTariffDiscount[] {
  return [
...
export function parseTokyoGasTariffApiPayload(value: unknown, { billingMonth, plan = "enefarm" }: ImportOptions = {}): GasTariff {
  ...
  const source = value as JsonRecord | null;
  ...
  const bands = (Array.isArray(period?.hyo) ? period.hyo : []).map((row: JsonRecord) => {
  ...
  }).filter((band: GasTariffBand) => band.yenPerM3 > 0);
...
export async function importTokyoGasTariff({ billingMonth, readingDay = 1, region = "tokyo", plan = "enefarm", fetchImpl = fetch }: ImportOptions = {}) {
  if (!validBillingMonth(billingMonth)) throw new Error("billingMonth must be YYYY-MM");
...
export async function importGasTariff(provider: string, options: ImportOptions = {}) {
  const adapter = GAS_TARIFF_PROVIDERS[provider as keyof typeof GAS_TARIFF_PROVIDERS];
  if (!adapter) throw new Error(`No automatic tariff adapter is available for ${provider}`);
  return adapter.importMonthly(options);
}

export function applicableGasTariffBand(tariff: GasTariff | null | undefined, expectedMonthlyM3: unknown): GasTariffBand | null {
...
export function applicableGasDiscount(tariff: GasTariff | null | undefined, discountId: string): GasTariffDiscount | null {
