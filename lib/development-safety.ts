...
export function externalIoDisabled(environment: Environment = process.env): boolean {
  return enabled(environment.DISABLE_EXTERNAL_IO);
}

export function uiDevelopmentMode(environment: Environment = process.env): boolean {
...
export function assertSafeUiDevelopmentEnvironment(
  ...
  }: { projectDir?: string; temporaryRoot?: string } = {},
): void {
